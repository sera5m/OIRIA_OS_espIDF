...
class VulcanIde(Gtk.Application):
    ...
    def _build(self):
        ...
        for label, cb in (("Open", self._on_open), ("Save", self._on_save),
                          ("New", self._on_new)):
    ...
    def _build_tyrant(self):
        ...
        hint = Gtk.Label(
            label="PC is puppeteer. Watch obeys TY lines and framed exec.",
            wrap=True, xalign=0,
        )
    ...
    def _on_target_changed(self, *_):
        t = self._target()
        self._touch_status()
        self._append_log(f"[target] {'This PC' if t == 'pc' else 'Watch'}\n")

    def _on_tyrant_toggled(self, btn):
        on = btn.get_active()
        self.tyrant_box.set_sensitive(on)
        self._append_log("[tyrant] %s\n" % ("ON — PC orders the watch" if on else "off"))
        self._touch_status()

    def _touch_status(self):
        bits = []
        bits.append("connected" if self.link.open else "disconnected")
    ...
    def _on_connect(self, *_):
        ...
        try:
            ...
            self.status.set_text(f"fail: {e}")
            self._append_log(f"[connect] {e}\n")
            return
        self._touch_status()
        self._append_log(f"[connect] {port} {baud}\n")
        try:
            self.link.write(b"\nTY PING\n")
        except Exception:
            pass
    ...
    def _run_local(self, src):
        ...
        if not runner:
            self._append_log(
                "[pc] no local interpreter (rsvm/vulcan not on PATH). "
                "Switch target to Watch, or turn on Tyrant mode to order the device.\n"
            )
            self.status.set_text("no local rsvm — use Watch / Tyrant")
            return
        tmp = Path("/tmp/vulcan_ide_run.vul")
        tmp.write_text(src, encoding="utf-8")
        self._append_log(f"[pc] {runner} {tmp}\n")
        ...
        def work():
            try:
                ...
                GLib.idle_add(self.status.set_text, f"pc exit {p.returncode}")
            except Exception as e:
                GLib.idle_add(self._append_log, f"[pc] {e}\n")
    ...
    def _on_io_write(self, *_):
        ...
        self._ty_send(f"TY IO W {pin} {val}\n")

    def _on_io_read(self, *_):
        pin, _ = self._pin_val()
        self._ty_send(f"TY IO R {pin}\n")
    ...
    def _open_path(self, path):
        ...
        self._set_text(text)
        self.path = path
        self.win.set_title(f"Vulcan IDE — {Path(path).name}")
        self._touch_status()

    def _on_save(self, *_):
        if self.path:
            Path(self.path).write_text(self._get_text(), encoding="utf-8")
            self.status.set_text(f"saved {self.path}")
            self._refresh_files()
            return
    ...
    def _on_new(self, *_):
        ...
        n = 1
        while True:
            p = self.workdir / f"untitled{n}.vul"
            if not p.exists():
                break
            n += 1
