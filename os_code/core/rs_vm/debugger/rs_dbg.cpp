...
void rsvm_dbg_error(const rsvm_t* vm, int line, int col, const char* msg) {
    ...
    if (vm->last_status == RSVM_ERR_STEP_LIMIT)
        fprintf(stderr, " (steps=%u limit=%u)", vm->steps, vm->step_limit);
    if (msg && msg[0])
...
void rsvm_dbg_stats(const rsvm_t* vm, FILE* out) {
    ...
    fprintf(out, "  steps      : %u / %u\n", vm->steps, vm->step_limit);
    fprintf(out, "  code_len   : %u\n", vm->code_len);
    fprintf(out, "  entry_pc   : %u\n", vm->entry);
    fprintf(out, "  slots      : %u\n", vm->slot_count);
    fprintf(out, "  funcs      : %u\n", vm->func_count);
    fprintf(out, "  structs    : %u\n", vm->struct_count);
    fprintf(out, "  enums      : %u\n", vm->enum_count);
    fprintf(out, "  flows      : %u\n", vm->flow_count);
    fprintf(out, "  heap_used  : %u / %u\n", vm->heap_used, (unsigned)RSVM_HEAP_BYTES);
    fprintf(out, "  str_pool   : %u / %u\n", vm->str_pool_used, (unsigned)RSVM_STR_POOL);
    fprintf(out, "  refs       : %u\n", vm->ref_count);
...
void rsvm_dbg_refs(const rsvm_t* vm, FILE* out) {
    ...
    for (uint8_t i = 0; i < vm->ref_count; ++i) {
        ...
        const char* k = r->kind == RSVM_REF_FUNC ? "func" :
...
void rsvm_dbg_monitors(const rsvm_t* vm, FILE* out) {
    ...
    for (uint8_t i = 0; i < vm->func_count; ++i) {
        ...
        if (!(p & (RSVM_PROP_MON_TIME | RSVM_PROP_MON_TIME_HI | RSVM_PROP_MON_RAM | RSVM_PROP_MON_EXECS)))
            continue;
        if (!any) { fprintf(out, "---- monitors ----\n"); any = 1; }
        fprintf(out, "  fn %s:\n", vm->funcs[i].name);
        if (p & RSVM_PROP_MON_EXECS)
            fprintf(out, "    execs     : %u\n", vm->func_execs[i]);
        if (p & RSVM_PROP_MON_TIME)
            fprintf(out, "    last_ms   : %u\n", vm->func_last_ms[i]);
        if (p & RSVM_PROP_MON_TIME_HI)
            fprintf(out, "    last_us   : %u\n", vm->func_last_us[i]);
        if (p & RSVM_PROP_MON_RAM)
            fprintf(out, "    last_ram  : %u (heap+str delta)\n", vm->func_last_ram[i]);
    }
    if (any) fprintf(out, "------------------\n");
}
